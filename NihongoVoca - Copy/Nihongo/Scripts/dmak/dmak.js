...
; (function () {
    ...
    Dmak.options = {
        uri: "http://drawmeakanji.com/kanji/",
        skipLoad: false,
        autoplay: true,
        height: 109,
        width: 109,
    ...
    Dmak.fn = Dmak.prototype = {
        ...
        render: function (end) {
            ...
            var cb = function (that) {
                ...
                // Execute custom callback "done" here
                if (that.pointer == that.kanjis.length - 1) {
                    that.options.done(that.pointer);
                }
...
; (function () {
    ...
    function loadSvg(uri, index, charCode, callbacks) {
        ...
        var xhr = createCORSRequest('GET', uri + code + ".svg");
        if (!xhr) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    loadSvg(uri, index, charCode, callbacks);
...
$(document).ready(function () {
    ...
    $('.drawKanji').on('click', function () {
        $('#txtKanji').val($(this).text());
        dr($(this).text());
        $('#drawingModal').modal();
        return false;
    })
